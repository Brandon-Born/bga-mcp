...
Ten tools and eleven concrete resources are discoverable, and none of them is verified. Every acceptance case each one claims is mapped to an assertion against the installed package, and the stdio transport is verified on protocol `2025-11-25`. The control that held all of them back — successful results were not minimized or passed through redaction — landed on 2026-08-10 (BGA-327), so the [threat model](docs/THREAT_MODEL.md) no longer records an open surface that every result crosses. What each capability still needs is a passing CI run of the commit that carries it. The Studio log reader stays experimental and not live-verified; BGA-320, BGA-322, BGA-326, and BGA-328 remain open against it, while BGA-319, BGA-321, and BGA-327 closed its output-privacy defects. The corrections to search accounting (BGA-209), framework-version reading (BGA-210), and address normalization (BGA-323) are verified as corrections, but the documentation capabilities that use them are not: their maintained retrieval evaluation still fails on excerpt selection, and BGA-324, BGA-326, and BGA-328 hold recorded defects open against the boundaries they run on. See the [2026-08-08 adversarial review](docs/verification/ADVERSARIAL_REVIEW_2026-08-08.md) and the [implementation backlog](docs/BACKLOG.md).
...
| `--studio-session-file <path>` | Read the Studio session from a file instead of `BGA_STUDIO_SESSION`; do not use with a real credential until BGA-328 bounds the read and stops printing the path. |
