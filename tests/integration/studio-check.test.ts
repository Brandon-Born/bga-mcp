...
describe('studio setup check', () => {
  ...
  it('[INT-STUDIO-CHECK-GUIDANCE] reads the session from a file when one is configured', async () => {
    ...
    expect(report.lines.some((entry) => entry.text.includes(file))).toBe(true);
