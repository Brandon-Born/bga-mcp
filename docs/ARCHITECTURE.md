...
This component is implemented in `src/policy.ts`, and the current production source has no privileged-effect import outside it. `GATE-POLICY-COMPLETE-EFFECT-BOUNDARY` parses every production module and refuses any spelling that can name a privileged builtin — bare, `node:`, subpath, re-export, `import()`, `require()` — along with the globals that reach the network without importing anything, on an allowlist that fails closed. ESLint states the same rule for immediate feedback, and the two are compared. Static roots and links are checked before reads, but BGA-330 owns binding those checks to the exact opened object and bounding every encountered entry. Defaults are local, read-only, and network-off.
