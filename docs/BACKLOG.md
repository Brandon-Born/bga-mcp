...
- **Progress, 2026-08-10:** BGA-321 closed the file-sourced session value: every provider registers the exact value it resolved before anything is published. The printed session path and the unbounded file read (BGA-328) remain open, so this item stays `implemented`.
...
- **Not covered:** `--studio-check` still prints the configured session-file path, and the file is still read without a type, size, owner, or mode check. Both are BGA-328's, and `TM-STUDIO-FILE-SESSION-BLOCKED` keeps `SURFACE-CLI-STDOUT` open until it lands.
