...
- **Remaining evidence limit:** The installed filesystem probe deliberately covers listing and descriptor-read syscalls, not every native filesystem primitive, and Node cannot cancel an already-issued `open`, `stat`, `opendir`, or `read` syscall. Cooperative work is awaited within the 250 ms cleanup ceiling; a primitive that outlives it remains a recorded platform residual rather than turning the public deadline into an unbounded wait. There is still no installed-artifact slow-parser oracle, and native regex, `JSON.parse`, and sorting are atomic segments bounded by input budgets and followed by checkpoints rather than interruptible mid-engine call. `E2E-POLICY-CANCELLATION` and the broader `E2E-STUDIO-READ-CANCELLATION` remain reserved for that complete matrix. The item is therefore `implemented`, not `verified`.
...
- **Note, 2026-08-13:** Exact-HEAD [CI run 31448712331](https://github.com/Brandon-Born/bga-mcp/actions/runs/31448712331) passed the earlier packaged cases. This item remains `implemented` because the acceptance map still records the installed-artifact synchronization barrier as missing, while descriptor-relative `openat` remains an explicit residual risk rather than a claimed guarantee. The new integration scenarios have passed locally but do not inherit that older exact-HEAD CI evidence.
- **Not covered:** The deterministic in-operation barriers run against `PolicyBoundary`, not an installed MCP server. The packaged scenario still arranges mutations between client calls and does not record the installed package's `lstat`/`open`, descriptor `stat`/`read`, or `lstat`/`opendir` stage order. The acceptance map therefore keeps the installed-artifact barrier explicitly missing; a test-only module customization can supply that evidence without adding a production hook. Portable Node pathname checks also cannot prevent repeated adversarial swaps across every successive check; `RR-POLICY-TRAVERSAL-OPENAT` records that residual honestly.
