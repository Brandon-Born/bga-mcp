...
- **Verification state, 2026-08-14:** Exact-HEAD [CI run 31448712331](https://github.com/Brandon-Born/bga-mcp/actions/runs/31448712331) passed the earlier packaged cases. Full local `pnpm check` now passes with 71 test files and 564 tests, and the acceptance map records 143 of 145 cases proven with only the two live/second-account Studio cases still missing. The new installed barrier does not inherit the older hosted run, so BGA-330 remains `implemented` until exact-HEAD CI passes. Portable Node pathname checks still cannot prevent repeated adversarial swaps across every successive check; `RR-POLICY-TRAVERSAL-OPENAT` records that residual honestly rather than treating deterministic single-swap evidence as `openat` equivalence.
...
- **Evidence, 2026-08-14:** [`tests/e2e/race-barrier-stub.ts`](../tests/e2e/race-barrier-stub.ts) is a preload-only test module outside the tarball. [`E2E-POLICY-RACE-BARRIER`](../tests/e2e/race-barrier.test.ts) proves the public call is pending at each barrier, releases the child to perform the mutation between the named real filesystem stages, requires `policy.path.symlink-escape`, scans every public and retained surface for all canaries, restores each fixture, and successfully calls `check_setup` through the same client. It also scans the installed package for the test filename and environment prefix, so the proof does not add a production backdoor. Full local `pnpm check` passes with 71 test files and 564 tests; all 164 required scenarios pass, and the acceptance map records 143 of 145 cases proven. Exact-HEAD CI is still required before this item or BGA-330 becomes `verified`.
