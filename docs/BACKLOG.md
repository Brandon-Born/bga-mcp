...
- **Local verification:** Full `pnpm check` passes with 70 test files and 563 tests, including the active-checkpoint and neutralized-checkpoint installed-package cases; the acceptance map now records 141 of 145 cases proven and four explicit gaps. The six-job CI matrix has not run on a commit containing this evidence, so the item remains `implemented` rather than `verified`.
