...
- **Note:** `implemented`. The verification this item asked for now exists: `E2E-FRAMEWORK-VERSION-CURRENT`, `E2E-FRAMEWORK-VERSION-STALE`, `E2E-FRAMEWORK-VERSION-MISSING`, and `E2E-FRAMEWORK-VERSION-CONFLICTING` read captured snapshots of the official page through the installed artifact, and `pnpm test:framework-version` repeats the reading against the live page before a documentation release. What the item still lacks is the CI run of the commit that added them.
...
- **Note:** `implemented`, not `verified`: the gates pass locally, and the CI run of this commit is the evidence the manifest still needs. Nothing here is evidence about TLS, the address guard, or name resolution — the stub replaces the connection those controls act on, and they keep their own scenarios.
...
- **Note:** `implemented`, not `verified`: the CI run of this commit is the evidence the manifest still needs. The live check is deliberately outside `pnpm check`, like the documentation evaluation, because a commit gate that depends on a third party's uptime fails for reasons that have nothing to do with the commit.
...
- **Note:** `implemented`, not `verified`: the CI run of this commit is the evidence the manifest still needs. TM-DOC-ADDRESS-NORMALIZATION and TM-STUDIO-READ-ADDRESS-NORMALIZATION move from `planned` to `implemented`.
