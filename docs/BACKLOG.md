...
- **Not covered:** `--studio-check` still prints the configured session-file path, which BGA-328 owns. Own-account lines that carry an unrecognized credential shape are BGA-327's.
